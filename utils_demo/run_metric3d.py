...
def extract_mono_geo_demo(img_name_list, color_images, color_intrinsics, model_type='metric3d_vit_large'):
    trans_topil = transforms.ToPILImage()
    # build model
    ...
    for i, (img_name, rgb_origin, intrinsic_matrix) in enumerate(zip(img_name_list, color_images, color_intrinsics)):
        # Convert RGBA to RGB if necessary
        if rgb_origin.shape[2] == 4:
            print(f"Warning: Image {img_name} has 4 channels (RGBA), converting to RGB")
            rgb_origin = rgb_origin[:, :, :3]  # Take only RGB channels
        elif rgb_origin.shape[2] != 3:
            raise ValueError(f"Image {img_name} has {rgb_origin.shape[2]} channels, expected 3 (RGB)")
        ...
        # plt.imsave(save_path, pred_depth.detach().cpu().squeeze(), cmap='jet')
...
def predict_mono_geo_demo(img_name_list, color_images, color_intrinsics, depth_save_dir, normal_save_dir, model_type='metric3d_vit_large'):
    """
    This function is the same as extract_mono_geo_demo(), it's just that this one doesn't return all images in arrays.
    """
    ...
    for img_name, rgb_origin, intrinsic_matrix in zip(img_name_list, color_images, color_intrinsics):
        # Convert RGBA to RGB if necessary
        if rgb_origin.shape[2] == 4:
            print(f"Warning: Image {img_name} has 4 channels (RGBA), converting to RGB")
            rgb_origin = rgb_origin[:, :, :3]  # Take only RGB channels
        ...
        # save depth
        save_path = os.path.join(depth_save_dir, f'{img_name}')
        save_npy_path = save_path.rsplit('.', 1)[0] + '.npy'
        np.save(save_npy_path, pred_depth.detach().cpu().numpy())
        # plt.imsave(save_path, pred_depth.detach().cpu().squeeze(), cmap='jet')
        # print(f'save to {save_path}')
        ...
        # save normals
        save_path = os.path.join(normal_save_dir, f'{img_name}')
        save_npy_path = save_path.rsplit('.', 1)[0] + '.npy'
        np.save(save_npy_path, pred_normal_resized_normalized.detach().cpu().numpy())
        # trans_topil(pred_normal_resized_normalized).save(save_path)
        # print(f'save to {save_path}')
        ...
        # del pred_depth, pred_normal, pred_normal_resized, pred_normal_resized_normalized
    
    del model
    torch.cuda.empty_cache()
    return depth_paths_list, normal_paths_list

def predict_masked_mono_geo(img_name_list, color_images, fg_masks, color_intrinsics, depth_save_dir, normal_save_dir, model_type='metric3d_vit_large'):
    """
    This function is the same as extract_mono_geo_demo(), it's just that this one doesn't return all images in arrays.
    """
    ...
    for img_name, rgb_origin, intrinsic_matrix, fg_mask in zip(img_name_list, color_images, color_intrinsics, fg_masks):
        # Convert RGBA to RGB if necessary
        if rgb_origin.shape[2] == 4:
            print(f"Warning: Image {img_name} has 4 channels (RGBA), converting to RGB")
            rgb_origin = rgb_origin[:, :, :3]  # Take only RGB channels
        ...
        # mask
        this_mask = torch.from_numpy(fg_mask)
        pred_normal_resized_normalized *= this_mask[None, :, :].cuda()
