...
class PlanarRecWrapper(nn.Module):
    def __init__(self, conf, plane_plots_dir: str):
        ...
        self.planarSplat = get_class(self.plane_model_conf.get_string('plane_model_class'))(conf, plot_dir=plane_plots_dir)
    ...
    def split_large_plane(self, radii_threshold=0.1):
        self.regularize_plane_shape()
        _, _, plane_center, plane_radii, _, plane_xAxis, plane_yAxis = self.planarSplat.get_plane_geometry()
        plane_num = self.planarSplat.get_plane_num()
        plane_radii_xy_p = plane_radii[:, :2]
        ...
        x_split_mask = plane_radii[:, 0] > radii_threshold
        y_split_mask = plane_radii[:, 1] > radii_threshold
        selected_mask_1 = torch.logical_and(y_split_mask, ~x_split_mask)
        selected_mask_2 = torch.logical_and(x_split_mask, ~y_split_mask)
        selected_mask_3 = torch.logical_and(x_split_mask, y_split_mask)
        selected_mask = selected_mask_1 | selected_mask_2 | selected_mask_3
    ...
    def split_selected_planes(self, split_mask):
        if split_mask.sum() == 0:
            return

        self.regularize_plane_shape(empty_cache=False)
        _, _, plane_center, plane_radii, _, plane_xAxis, plane_yAxis = self.planarSplat.get_plane_geometry()
        plane_radii_xy_p = plane_radii[:, :2]
        plane_radii_xy_n = plane_radii[:, 2:]
        plane_rot_q_normal_wxy = self.planarSplat.get_plane_rot_q_normal_wxy
        ...
        new_plane_center, new_plane_radii_xy_p, new_plane_radii_xy_n, new_plane_rot_q_normal_wxy, new_plane_rot_q_xyAxis_w, new_plane_rot_q_xyAxis_z, new_rot_delta = model_util.split_planes_xy_via_mask(
            split_mask,
            plane_xAxis,
            plane_yAxis,
            plane_center,
            plane_radii_xy_p,
            plane_radii_xy_n,
            plane_rot_q_normal_wxy,
            plane_rot_q_xyAxis_w,
            plane_rot_q_xyAxis_z,
            self.planarSplat.rot_delta
        )
        ...
        # Important: align vis bookkeeping with the new plane count.
        self.reset_plane_vis()

        prune_mask = torch.cat(
            (split_mask, torch.zeros(new_plane_center.shape[0], device="cuda", dtype=bool))
        )
        self.prune_core(prune_mask)
        torch.cuda.empty_cache()
    ...
    def update_plane_vis(self, vis_mask=None):
        # The logic here:
        # [Has non-zero gradient] -> [The plane survives]
        # That is a necessary condition for keeping a plane, but not a sufficient one.
        #
        # Common failure cases:
        # A wrong plane can still reduce loss a little, so it gets gradient and survives.
        # A background or floating artifact plane can receive gradient through depth/normal supervision noise.
        # Redundant overlapping planes can all get gradient and all survive.
        # Early in training, gradients can be noisy, so “seen once” is weak.
        #
        # So a better keep/prune rule is usually multi-criterion, for example: 
        # Foreground support: enough consistent FG hits across views.
        # Background penalty: prune if it activates on BG rays too often.
        # Geometric sanity: min/max size, normal consistency, reprojection consistency.
        # Redundancy control: overlap/merge checks.
        # Temporal stability: require support over multiple visibility-check rounds.
