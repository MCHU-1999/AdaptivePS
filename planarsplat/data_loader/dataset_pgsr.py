...
class ViewInfo(nn.Module):
    ...
    def _load_depth(self):
        ...
        if self._mono_depth_cache is None:
            ...
            # Resize depth map to match img_res if needed
            if depth.shape != (self.img_res[0], self.img_res[1]):
                depth_tensor = torch.from_numpy(depth).float()  # h, w
                depth_resized = F.interpolate(
                    depth_tensor[None, None, :, :],
                    size=(self.img_res[0], self.img_res[1]), 
                    mode='bilinear'
                ).squeeze()
                self._mono_depth_cache = depth_resized.cuda().reshape(-1)  # hw
            else:
    ...
    def _load_normals(self):
        """Lazy load normal maps (H, W, 3) or (3, H, W) and transform to global"""
        if self._mono_normal_global_cache is None:
            normal = np.load(self.normal_path)  # expected: (H, W, 3) or (3, H, W)
            
            # Check the shape and adjust if necessary
            if normal.ndim != 3 or (normal.shape[2] != 3 and normal.shape[0] != 3):
                raise ValueError(f"Expected normal shape (H, W, 3) or (3, H, W), got {normal.shape}")
            
            # If the format is (3, H, W), transpose to (H, W, 3)
            if normal.shape[0] == 3:
                normal = normal.transpose(1, 2, 0)  # (3, H, W) -> (H, W, 3)

            # HWC -> CHW for interpolation
            normal_tensor = torch.from_numpy(normal).float().permute(2, 0, 1)  # (3, H, W)
            if normal_tensor.shape[1:] != (self.img_res[0], self.img_res[1]):
                normal_tensor = F.interpolate(
                    normal_tensor.unsqueeze(0),  # (1, 3, H, W)
                    size=(self.img_res[0], self.img_res[1]),
                    mode='bilinear',
                    align_corners=False
                ).squeeze(0)  # (3, H, W)
            ...
            # CHW -> HWC
            normal_local = normal_tensor.permute(1, 2, 0).cuda().float()  # (H, W, 3)
            # Transform to global coordinates, then flatten
            normal_global = (normal_local @ self.pose[:3, :3].T).view(-1, 3)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if mesh_pre_align:
            ...
            for i in tqdm(range(len(depth_paths)), desc='aligning depth...'):
                depth = np.load(depth_paths[i])
                img_name = os.path.basename(depth_paths[i]).rsplit('.', 1)[0]

                # Resize depth map to match img_res if needed
                if depth.shape != (img_res[0], img_res[1]):
                    depth_tensor = torch.from_numpy(depth).float()
                    depth_resized = F.interpolate(
                        depth_tensor[None, None, :, :], size=(img_res[0], img_res[1]), mode='bilinear'
                    ).squeeze()
                    depth_tensor = depth_resized.cuda()
                else:
                    depth_tensor = torch.from_numpy(depth).cuda().float()
                rd = rendered_depths[i].cuda()
                weight = ((depth_tensor.reshape(-1) - rd).abs() <= 0.5) & (rd > 0.05)
                d_scale = align_depth_scale(depth_tensor.reshape(1, -1), rd.reshape(1, -1), weight=weight.reshape(1, -1).float())
                if d_scale > 0:
                    depth_tensor = depth_tensor * d_scale.item()
                    depth_tensor = torch.clamp(depth_tensor, 0, 300)
                
                # Save aligned depth map
                aligned_depth_path = os.path.join(aligned_depth_dir, f'{img_name}.npy')
                np.save(aligned_depth_path, depth_tensor.cpu().numpy().astype(np.float32))
