...
def get_train_param(opt, conf):
    ...
    tag = opt['tag'] if opt['tag'] != '-1' else conf.get_string('dataset.tag', default='-1')
    if tag != '-1':
        expname = expname + '_{0}'.format(tag)
    if opt['is_continue']:
    ...
    return expname, tag, timestamp, is_continue
    # return expname, timestamp, is_continue
...
def put_if_not_none(conf, key, value):
