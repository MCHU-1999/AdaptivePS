...
class random_color(object):
    def __init__(self, color_num=40000):
        self.colors = []
        self._append_colors(color_num)

    def _append_colors(self, n_new):
        self.colors.extend(
            ["#" + "".join([random.choice("0123456789ABCDEF") for _ in range(6)]) for _ in range(n_new)]
        )

    def _ensure_size(self, needed):
        # needed is number of colors required, not max index
        if needed <= len(self.colors):
            return
        grow = needed - len(self.colors)
        # optional headroom to reduce frequent realloc
        grow = max(grow, len(self.colors))
        self._append_colors(grow)

    def __call__(self, ret_n=10):
        self._ensure_size(ret_n)
        ret_color = np.zeros([ret_n, 3], dtype=np.float32)
        for i in range(ret_n):
            hex_color = self.colors[i][1:]
            ret_color[i] = np.array([int(hex_color[j:j + 2], 16) for j in (0, 2, 4)], dtype=np.float32)
        ret_color[0] *= 0
        return ret_color
...
def plot_rectangle_planes(plane_centers, plane_normals, plane_radii, rot_q, epoch=-1, suffix='', out_path=None, plane_id=None, color_type=''):
    ...
    if color_type == 'normal':
        ...
        category_colors = np.array([
            [255,  64,  64],  # fg_only | RED
            [ 64, 128, 255],  # bg_only | BLUE
            [ 64, 220, 128],  # both    | GREEN
            [180, 180, 180],  # neither | GREY
