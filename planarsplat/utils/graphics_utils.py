...
def normal_from_depth_image(depth, intrinsic_matrix, extrinsic_matrix, offset=None, gt_image=None):
    ...
    xyz_normal = xyz_normal.permute(2,0,1) # Shape: (H, W, 3) -> (3, H, W)

    return xyz_normal

def render_normals(depths, extrinsics, intrinsics):
    # depth: (H, W), bg_color: (3), alpha: (H, W)
    # normal_from_depth: (3, H, W)
    normals = []
    for depth, intrinsic, extrinsic in zip(depths, intrinsics, extrinsics):
        normal_from_depth = normal_from_depth_image(depth, intrinsic.to(depth.device), extrinsic.to(depth.device))
        normals.append(normal_from_depth.cpu().numpy())  # Convert to numpy and add to list

    # Stack all normals into a single numpy array
    normals = np.stack(normals, axis=0)  # Shape: (N, 3, H, W)
