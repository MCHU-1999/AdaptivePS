...
def _run_adaptiveps(
    ...
    colmap_cam_file_path = os.path.join(data_path, 'DA3_aligned/cameras.bin')
    if not os.path.exists(colmap_cam_file_path):
        colmap_cam_file_path = os.path.join(data_path, 'DA3_aligned/cameras.txt')
        if not os.path.exists(colmap_cam_file_path):
            raise ValueError(f'The input path {colmap_cam_file_path} does not exist.')

    colmap_image_file_path = os.path.join(data_path, 'DA3_aligned/images.bin')
    if not os.path.exists(colmap_image_file_path):
        colmap_image_file_path = os.path.join(data_path, 'DA3_aligned/images.txt')
        if not os.path.exists(colmap_image_file_path):
